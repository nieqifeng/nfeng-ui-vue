  ...
  <div class="properties-centent kk-checkbox">
    ...
    <div class="properties-body">
      ...
      <a-form v-show="selectItem.key !== ''">
        ...
        <a-form-item
          label="jsonSchema类型（必填）"
        >
          <a-select
            show-search
            v-model="selectItem.jsonSchemaType"
          >
            <a-select-option v-for="item in jsonSchemaTypeArr" :key="item.value" :value="item.value">
              {{item.text}}
            </a-select-option>
          </a-select>
          <a-select
            show-search
            v-if="selectItem.jsonSchemaType === 'array'"
            v-model="selectItem.arrayValueType"
          >
