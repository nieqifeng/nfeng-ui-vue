<template>
  <a-config-provider :locale="locale">
    <div class="form-designer-container-9136076486841527">
      <k-header v-if="showHead" :title.sync="title" @changeTitle="title => $emit('changeTitle', title)"/>
      <!-- 操作区域 start -->
      <operatingArea
        v-if="toolbarsTop"
...
export default {
  ...
  computed: {
    data() {
      ...
      let keys = Object.keys(designData)
      return keys.length ? designData : this.defaultData
    },
    basicsArray() {
      // 计算需要显示的基础字段
  ...
  methods: {
    ...
    handleReset() {
      ...
      this.$confirm({
        ...
        onOk: () => {
          this.data.list = [];
          this.handleSetSelectItem({ key: "", jsonSchemaType: 'string' });
